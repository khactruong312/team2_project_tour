<?php
// views/admin/Report/tour.php
?>
<!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="utf-8">
    <title>Doanh thu theo Tour</title>
...
<div class="container p-4">
    <h2 class="text-body mb-3">Doanh thu theo Tour</h2>

    <form class="row g-3 mb-3" action="index.php" method="get">
        <input type="hidden" name="act" value="report-tour">
        <div class="col-md-4">
            <label class="form-label">Từ ngày</label>
            <input type="date" class="form-control" name="from" value="<?= htmlspecialchars($_GET['from'] ?? date('Y-01-01')) ?>">
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4 d-flex align-items-end">
            <button class="btn btn-primary me-2">Xem báo cáo</button>
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-4">
            <div class="card p-3">
                <h6>Tổng chi phí</h6>
                <h3><?= number_format($totalExpense ?? 0) ?> VNĐ</h3>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card p-3">
                <h6>Tổng lợi nhuận</h6>
                <?php $profit = ($totalRevenue ?? 0) - ($totalExpense ?? 0); ?>
    ...
    <div class="card">
        <div class="card-header">Bảng chi tiết theo tour</div>
        <div class="card-body">
...
<script>
    ...
    const labels = raw.map(r => r.tour_name);
    const revenues = raw.map(r => Number(r.total_revenue));
    const expenses = raw.map(r => Number(r.total_expense));

    const ctx = document.getElementById('chartTour').getContext('2d');
    new Chart(ctx, {
        type: 'bar',
        data: {
            labels,
