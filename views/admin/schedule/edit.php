<div class="container p-4">
            ...
            <select name="tour_id" class="form-select" required>
                <?php foreach ($tours as $tour): ?>
                    <option value="<?= $tour['tour_id'] ?>"
                        <?= $tour['tour_id'] == $schedule['tour_id'] ? 'selected' : '' ?>>
                        <?= $tour['name'] ?>
                    </option>
                <?php endforeach; ?>
            </select>
            ...
            <select name="guide_id" class="form-select">
                ...
                <?php foreach ($guides as $g): ?>
                    <option value="<?= $g['guide_id'] ?>"
                        <?= $g['guide_id'] == $schedule['guide_id'] ? 'selected' : '' ?>>
                        <?= $g['full_name'] ?>
                    </option>
                <?php endforeach; ?>
            </select>
    ...
    <select name="status" class="form-select">
        <option value="Chưa khởi hành" 
            <?= $schedule['status'] == 'Chưa khởi hành' ? 'selected' : '' ?>>
            Chưa khởi hành
        </option>

        <option value="Đang chạy" 
            <?= $schedule['status'] == 'Đang chạy' ? 'selected' : '' ?>>
            Đang chạy
        </option>

        <option value="Hoàn thành" 
            <?= $schedule['status'] == 'Hoàn thành' ? 'selected' : '' ?>>
            Hoàn thành
        </option>
