...
<body>
    ...
    <h1 class="mt-4 mb-4 text-secondary">
        <i class="fas fa-list me-2"></i> Danh Sách Tour Của Tôi
    </h1>

    <div class="card shadow mb-4">
        <div class="card-header bg-primary text-white fw-bold">
            <i class="fas fa-route me-2"></i> Tất cả Tour được phân công
        </div>
                    ...
                    <tbody>
                        <?php foreach ($bookings as $tour) : ?>
                            <tr>
                                ...
                                <td>
                                    <img src="./uploads/imgproduct/<?= $tour['image'] ?>" 
                                         alt="<?= $tour['name'] ?>" 
                                ...
                                <td>
                                    <?php 
                                        $badge = "bg-secondary";
                                        if ($tour['tour_status'] == "Active") $badge = "bg-success";
                                        if ($tour['tour_status'] == "Inactive") $badge = "bg-danger";
                                    ?>
                                ...
                                <td>
                                    <?php
                                        $status = $tour['schedule_status'];
                                        $badgeRun = "bg-secondary";

                                        if ($status == "Đang chạy") $badgeRun = "bg-info";
                                        if ($status == "Sắp khởi hành") $badgeRun = "bg-warning text-dark";
                                        if ($status == "Hoàn thành") $badgeRun = "bg-success";
                                    ?>
                                    <span class="badge <?= $badgeRun ?> p-2">
                                        <?= $tour['schedule_status'] ?>
                                    </span>
                                </td>

                                <td>
                                    <a href="index.php?act=guide-progress&id=<?= $tour['schedule_id'] ?>" 
                                       class="btn btn-primary btn-sm mb-1">
                                        <i class="fas fa-edit"></i> Tiến trình
                                    </a>

                                    <a href="index.php?act=guide-report&id=<?= $tour['schedule_id'] ?>" 
...
<?php else : ?>
    <tbody>
        <tr>
            <td colspan="11" class="text-center text-danger fw-bold py-4">
                <i class="fas fa-exclamation-triangle me-2"></i> Không có Tour nào được phân công hoặc lỗi dữ liệu.
            </td>
        </tr>
    </tbody>
<?php endif; ?>
